...
# ---------------------------------------------------------------------------
# Synchronous MongoDB connection (bypasses Motor / asyncio entirely)
# ---------------------------------------------------------------------------
_sync_db = None

def _get_sync_db():
    """Get a synchronous pymongo database instance (lazy singleton)."""
    global _sync_db
    ...
    # Determine DB name (same logic as mongodb.py)
    ...
    logger.info(f"✅ Sync pymongo connected to database: {db_name}")
    return _sync_db


def update_job_status_sync(job_id: str, status: str, progress: int, clips: list = None, error: str = None):
    """Update job status in MongoDB using synchronous pymongo (no asyncio needed)."""
    db = _get_sync_db()
...
celery_app.conf.update(
    ...
    timezone="UTC",
    enable_utc=True,
    task_always_eager=not USE_ASYNC,
...
def process_video_task(self, job_id: str, video_source: str, source_type: str, settings: dict = None):
    ...
    try:
        ...
        update_job_status_sync(job_id, "processing", 20)
        ...
        update_job_status_sync(job_id, "processing", 90)

        # Step 6: Update job status to completed
        logger.info(f"✅ Job {job_id}: Step 6/6 - Finalizing job status")
        update_job_status_sync(job_id, "completed", 100, clips=clips)
